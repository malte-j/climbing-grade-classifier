...
data = numpy.genfromtxt("circle.csv", delimiter=",", skip_header=1)

sample_rate = 120  # 400 Hz

timestamp = data[:, 1]

timestamp = (timestamp - timestamp[0]) / 10000  # convert ms to s
...
figure, axes = pyplot.subplots(
    nrows=6, sharex=True, gridspec_kw={"height_ratios": [6, 6, 6, 2, 1, 1]}
)
...
axes[4].plot(
    timestamp, internal_states[:, 1], "tab:cyan", label="Accelerometer ignored"
)
pyplot.sca(axes[4])
...
_, axes = pyplot.subplots(
    nrows=4, sharex=True, gridspec_kw={"height_ratios": [6, 1, 6, 6]}
)
...
for index in range(len(timestamp)):
    is_moving[index] = (
        numpy.sqrt(acceleration[index].dot(acceleration[index])) > 3
    )  # threshold = 3 m/s/s
